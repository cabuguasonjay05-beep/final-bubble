...
async function getSupabaseBusinessProfile(): Promise<BusinessProfile> {
  const rows = await restRequest<SettingsRow[]>(
    "settings?key=eq.business_profile&select=key,value&limit=1",
  );
  return normalizeBusinessProfile(rows[0]?.value);
}

async function saveSupabaseBusinessProfile(profile: BusinessProfile): Promise<BusinessProfile> {
  const rows = await restRequest<SettingsRow[]>("settings", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify([
      {
        ...
        value: normalizeBusinessProfile(profile),
      },
    ]),
  });
